...
fn read_crypto_command<I: Iterator<Item = String>>(
    ...
    if target.is_empty() {
        ...
        let t = match is_path {
            true => EncryptionTarget::new_file(PathBuf::from(target).clean()),
            false => EncryptionTarget::new_text(target),
        };
...
impl EncryptionTarget {
    pub fn new_file(path: PathBuf) -> EncryptionTarget {
        EncryptionTarget::File(path)
    }

    pub fn new_text(s: String) -> EncryptionTarget {
        EncryptionTarget::Text(s)
    }
}

impl From<PathBuf> for EncryptionTarget {
    fn from(v: PathBuf) -> Self {
        EncryptionTarget::new_file(v)
    }
}

impl From<String> for EncryptionTarget {
    fn from(v: String) -> Self {
        EncryptionTarget::new_text(v)
    }
}
